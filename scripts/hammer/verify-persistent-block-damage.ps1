...
Assert-Contains $progressSource "this.id = renderIds.allocate()" "Cracks must allocate unique display IDs."
Assert-Contains $progressSource "renderIds.release(id)" "Removed cracks must release their display IDs."
Assert-NotContains $progressSource "HammerMining.crackId" "Position hashes can collide and must not identify overlays."
Assert-Contains $progressSource "!(state.getBlock() instanceof LiquidBlock)" "Actual fluid blocks must be excluded from saved damage."
Assert-NotContains $progressSource "state.getFluidState().isEmpty()" "Waterlogged solid blocks must retain damage."
...
Assert-Contains $mixinConfig '"hammer.GameModeDestroyAccessor"' "The mining progress accessor is not registered."
Assert-Contains $mixinConfig '"hammer.ServerBlockBreakingMixin"' "The persistent mining mixin is not registered."
